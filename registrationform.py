...
def database():
    ...
    mycursor.execute("insert into register values(%s,%s,%s,%s,%s,%s,%s)", (var.get(
    ), entry_2.get(), entry_3.get(), entry_4.get(), label_5.get()
    ,var.get()))
    messagebox.show.showinfo('info',"submitted")
    conn.commit()
root=Tk()
